document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateTabStyles() {
    const textColor = brightness <= 50 ? 'rgb(0, 0, 0)' : 'rgb(255, 255, 255)';
  ...
  function updateColors() {
    const buttonColor = `rgb(${255 - brightness * 2.55}, ${255 - brightness * 2.55}, ${255 - brightness * 2.55})`;
    const backgroundColor = `rgb(${brightness * 2.55}, ${brightness * 2.55}, ${brightness * 2.55})`;
    ...
    const containerColor = brightness <= 50
...
async function createChart(selectedMonth = null) {
  try {
    const jsonDataPath = "{{ url_for('static', filename='data.json') }}";
    const response = await fetch(jsonDataPath); // Перевірте шлях!
    const data = await response.json();

    const currentDate = new Date();
    const currentMonth = selectedMonth || `${currentDate.getFullYear()}-${(currentDate.getMonth() + 1).toString().padStart(2, '0')}`;

    // Фільтруємо дані за обраним місяцем
    const filteredData = data.filter(entry => {
      const entryMonth = `${entry.time.year}-${entry.time.month.toString().padStart(2, '0')}`;
    ...
    filteredData.forEach(entry => {
      const dayKey = `${entry.time.year}-${entry.time.month.toString().padStart(2, '0')}-${entry.time.day.toString().padStart(2, '0')}`;
      ...
      if (entry.action === "turn_on") {
        lastActionTime = new Date(entry.time.year, entry.time.month - 1, entry.time.day, entry.time.hour, entry.time.minute);
      }

      if (entry.action === "turn_off" && lastActionTime) {
        const turnOffTime = new Date(entry.time.year, entry.time.month - 1, entry.time.day, entry.time.hour, entry.time.minute);
        const durationInMinutes = (turnOffTime - lastActionTime) / (1000 * 60); // Тривалість у хвилинах
...
async function createDailyChart(selectedDate = null) {
  try {
    const response = await fetch('./data.json'); // Перевірте шлях!
    const data = await response.json();
    ...
    const filteredData = data.filter(entry => {
      const entryDate = `${entry.time.year}-${entry.time.month.toString().padStart(2, '0')}-${entry.time.day.toString().padStart(2, '0')}`;
...
function populateDayDropdown(selectedMonth = null) {
  ...
  const daysInMonth = new Date(currentMonth.split('-')[0], currentMonth.split('-')[1], 0).getDate(); // Кількість днів у місяці
  for (let day = 1; day <= daysInMonth; day++) {
    const normalizedDay = day.toString().padStart(2, '0');
    const optionValue = `${currentMonth}-${normalizedDay}`;
    const optionText = `${normalizedDay}/${currentMonth.split('-')[1]}/${currentMonth.split('-')[0]}`;
